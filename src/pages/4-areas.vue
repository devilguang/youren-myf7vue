...
<template>
  <f7-page>
    ...
    <f7-block style="margin-bottom: 50px">
      <!--href="/contact/"-->
      <f7-button round fill style="border-radius: 5px;margin-top: 30px" @click="nextStep()">确定</f7-button>
    </f7-block>
  </f7-page>
</template>
<script>
  export default {
    ...
    methods: {
      onChange($event, area, i) {
        ...
        //this.$set(this.areaMap[i], 'checked', $event.target.checked)
      },
      nextStep(){
        ...
        if (selectField.length == 0) {
          ...
          localStorage.setItem("registered", JSON.stringify(selectField));  //存储行业领域的信息
          let type = localStorage.getItem('type');
          if (type == "1") {
            this.$router.load({url: '/expertsHome'})
          } else {
            this.$router.load({url: '/certification'})
          }
        }
      },
    },
